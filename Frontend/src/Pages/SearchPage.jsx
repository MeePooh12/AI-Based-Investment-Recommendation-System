...
export default function SearchPage() {
  ...
  const t = {
    en: {
      title: "Stock Search",
      News: "News Daily",
      category: "There is no news in this category",
      loading: "Loading Data...",
      searchstock: "Search for stock names such as NVDA",
      favorites: "Your Favorites",
    },
    th: {
      title: "ค้นหาหุ้นที่คุณสนใจ",
      News: "ข่าววันนี้",
      category: "ไม่มีข่าวสารในหมวดหมู่นี้",
      loading: "กำลังโหลดข้อมูล...",
      searchstock: "ค้นหาชื่อหุ้น เช่น NVDA",
    ...
    const fetchNews = async () => {
      try {
        ...
        const responses = await Promise.all(
          stocks
            .slice(0, 4)
            .map((s) => fetch(`http://localhost:8000/stock/${s.symbol}`))
        );
        ...
        for (let res of results) {
          ...
          // 1️⃣ ข่าวจาก backend
          const apiNews = (
            res?.news && Array.isArray(res.news) ? res.news.slice(0, 2) : []
          ).map((n) => ({
            ...n,
            symbol: stockSymbol,
            name: stockName,
          ...
          try {
            const rssRes = await fetch(
              `http://localhost:8000/rss/${stockSymbol}`
            );
            ...
            console.warn(
              `Failed to fetch RSS news for ${stockSymbol}:`,
              rssErr
            );
            setNewsList([]);
          }
        ...
        // ลบข่าวซ้ำ (ตามลิงก์)
        const uniqueNews = [
          ...new Map(news.map((item) => [item.link, item])).values(),
        ];
        ...
        <div className="selected-stock max-w-md mx-auto p-4 border rounded-xl bg-white shadow-lg">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-gray-500 text-sm">
              {selectedStockData.sector || "Unknown Sector"}
            </span>
          </div>
            ...
            <p className="mb-2">
              ...
              <span
                className={`font-medium ${
                  selectedStockData.sentiment === "Positive"
                    ? "text-green-500"
                    : selectedStockData.sentiment === "Negative"
                    ? "text-red-500"
                    : "text-gray-500"
                }`}
              >
